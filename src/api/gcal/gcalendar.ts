...
const GCalendar = () => {
  ...
  fs.readFile(CREDENTIALS_PATH, (err, content) => {
    ...
    authorize(JSON.parse(content.toString()) as Credentials, listEvents);
  });
};
 ...
 */
const authorize = (credentials: Credentials, callback: Function) => {
...
const listEvents = (auth: OAuth2Client) => {
    ...
    (err: Error | null, res) => {
      ...
      if (events.length) {
        ...
        events.map((event: calendar_v3.Schema$Event, i: number) => {
          ...
          const separator = end ? " :: " : end;
          console.log(`${start}${separator}${end} - ${event.summary}`);
        });
      } else {
